...
class SlimUNETRBlock_v2(nn.Module):
    ...
    def forward(self, x,x_res):
        """
           ...
           处理3D特征图，结合Mamba序列建模和自注意力机制

           参数:
           - x: 输入张量, 形状为 [B, C, H, W, Z]
               B: batch size
               C: 通道数
               H, W, Z: 高度、宽度、深度
        ...
        assert C == self.dim  # 确保通道数符合预期
        # 计算空间维度的总token数 (H*W*Z)
        n_tokens = x.shape[2:].numel()
        # 保存空间维度 [H, W, Z]
        img_dims = x.shape[2:]

        # 4. 张量重塑和预处理
        # 将3D特征图展平为序列形式 [B, C, H*W*Z] -> [B, H*W*Z, C]
        ...
        # 7. 多头自注意力处理
        # B, C, H, W, Z = x.shape  # 获取当前特征图的具体维度
        B, C, H = x.shape
        # 生成Q, K, V张量
        q, k, v = self.qkv(out).view(B, self.num_heads, -1, H ).split(
            [self.head_dim, self.head_dim, self.head_dim],
            dim=2)
        # 计算注意力分数并应用softmax
        # (q.transpose(-2, -1) @ k) 计算Q和K的点积
        # .softmax(-1) 应用softmax得到注意力权重

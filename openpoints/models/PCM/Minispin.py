...
class MiniSpinNet(nn.Module):
    ...
    def forward(self, pts, kpts, z_axis=None, is_aug=False):
        ...
        # Vanilla SpinNet 特征提取网络处理
        new_points = inv_patches.permute(0, 3, 1, 2)  # (B, C_in, npoint, nsample+1), input features 调整维度顺序

        x = self.pnt_layer(new_points)# 最大池化
        ...
        x, mid = self.conv_net(x)   # 通过圆柱形卷积网络

        # 特征加权和归一化
        w = self.pool_layer(x) # 计算权重
        f = F.avg_pool2d(x * w, kernel_size=(x.shape[2], x.shape[3])) # 加权平均池化
        f = F.normalize(f.view(f.shape[0], -1), p=2, dim=1) # L2归一化
